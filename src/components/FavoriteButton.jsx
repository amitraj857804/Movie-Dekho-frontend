...
const FavoriteButton = ({ 
    ...
    const handleToggleFavorite = async (e) => {
        ...
        try {
            if (isFavorite) {
                ...
                await dispatch(addToFavorites(movieId)).unwrap();
            }
        } catch (error) {
    ...
    const getButtonClasses = () => {
        const baseClasses = "flex items-center justify-center transition-all duration-300 cursor-pointer";
    ...
    return (
        <button
            ...
            title={showTooltip ? (isFavorite ? 'Remove from favorites' : 'Add to favorites') : ''}
        >
            {isLoading ? (
                <div className={`animate-spin rounded-full border-2 border-white border-t-transparent ${size}`} />
            ) : (
                <>
                    {isFavorite ? (
